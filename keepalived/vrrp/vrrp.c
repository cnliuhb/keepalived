...
check_debounce_timers(vrrp_t *vrrp, unsigned advert_int)
{
	...
	if (IF_BASE_IFP(vrrp->ifp)->up_debounce_timer >= 2 * advert_int ||
	    IF_BASE_IFP(vrrp->ifp)->down_debounce_timer >= 2 * advert_int) {
		changed = true;
		if (IF_BASE_IFP(vrrp->ifp)->up_debounce_timer >= 2 * advert_int)
			IF_BASE_IFP(vrrp->ifp)->up_debounce_timer = 2 * advert_int - advert_int / 256;
		if (IF_BASE_IFP(vrrp->ifp)->down_debounce_timer >= 2 * advert_int)
			IF_BASE_IFP(vrrp->ifp)->down_debounce_timer = 2 * advert_int - advert_int / 256;
	}

#ifdef _HAVE_VRRP_VMAC_
	if (vrrp->ifp != vrrp->ifp->base_ifp) {
		if (vrrp->ifp->up_debounce_timer >= 2 * advert_int ||
		    vrrp->ifp->down_debounce_timer >= 2 * advert_int) {
			changed = true;
			if (vrrp->ifp->down_debounce_timer >= 2 * advert_int)
...
vrrp_complete_instance(vrrp_t * vrrp)
{
	...
	if (0) {
		...
		log_message(LOG_INFO, "%s: interface %s debounce timer(s) not less that %u * advert_int - resetting", vrrp->iname, vrrp->ifp->ifname, 2);
